...
For a basic queue with one server (M/M/1), the average time a request spends in the system (waiting and being processed) is `W = 1/(μ - λ)`. As the arrival rate (λ) gets closer to the service rate (μ), utilization approaches 100%, and the waiting time (W) approaches infinity.

Consider this:
*   At 50% utilization, the average wait time is twice the actual processing time.
*   At 80% utilization, it's 5 times the processing time.
*   At 95% utilization, it's a staggering 20 times the processing time!

This exponential increase means that trying to squeeze every last drop of performance out of your GPUs by keeping them at 95-100% utilization is a recipe for disaster. A tiny, unexpected spike in traffic can cause your wait times to explode, leading to system-wide slowdowns and unhappy users. The key is to design for a sustainable utilization level.
