...
For those of the initiated, who are aware of how docker system works and just wish for a no nonsence, TLDR
...
specific it is a specific kind of virtual network interface, one of many infact, that you can create in a linux system. This 
...
A network switch only connects devices to the same LAN. It has no routing capability, and infact it is required that all the devices
...
- The second way to achieve this is by performing routing on the kernelspace itself. This is infact the same way any network router, 
that runs linux would work. For this feature to work, you first have to enable IP packtes forwarding on your kernel. Without this 
feature enabled, any packet that reaches your linux machine will be droppped. With the feature enabled, the linux machine will
...
Let's assume that Contaienr B will have the server and Container A is to proxy it's request through host, then use the following 
...
heading into `172.17.0.3` should have their network address masqueraded. Masqueration basically changes the source IP of the packet,
to the packet of the linux machine. This is a crucial step, as without masqueration, the response sent by server will not come 
