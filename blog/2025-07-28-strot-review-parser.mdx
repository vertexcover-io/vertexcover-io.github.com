...
Introduction to Strot - The Ajax Scraper.
Any website that loads the data using ajax calls after the page load can be scraped using Strot. We show it's use case for reviews scraping below.

<TLDR>
Strot (Sanskrit meaning source) works in two steps:
...
Hence, an experiment is born - called `strot`. In sanskrit it means source. We are trying to get to the ajax calls that gets us reviews. 
...
Claims: 
Strot is the only project which can scrape the data available as ajax call - at all - using natural language.
